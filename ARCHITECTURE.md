...
| 模块 | 职责 |
|---|---|
| `CameraStreamManager` | 管理 N 路相机的 `AHardwareBuffer`，通过 `EGLImageKHR` 导入为 `GL_TEXTURE_EXTERNAL_OES`，处理相机线程→渲染线程的同步（EGL fence / `AHardwareBuffer_lock` 语义） |
...
2. **相机纹理零拷贝**：`AHardwareBuffer` + `EGL_ANDROID_get_native_client_buffer`
   / `eglCreateImageKHR` 导入 `GL_TEXTURE_EXTERNAL_OES`，避免 CPU 端 YUV→RGB。
...
```
[相机线程] CameraHAL/Camera2 callback → AHardwareBuffer 写入 → 通知渲染线程
[渲染线程] Choreographer vsync → Engine::tick() → Renderer::render(view) → eglSwapBuffers
[Job 线程池(可选,轻量)] 标定参数计算 / mesh 重建 / 异步资源上传（不参与每帧渲染路径）
```

渲染线程与相机线程之间只通过 Handle/Fence 同步，不做数据拷贝。Job 线程池
用简单的固定线程数 thread pool + lock-free/mutex 队列即可，不需要 Filament
那种复杂的 work-stealing JobSystem。
